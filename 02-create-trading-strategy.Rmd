...
We can pass our Binance account access details to our code using environment variables. Create a new `config/runtime.exs` at the root of our umbrella project:
...
Note that we use `runtime.exs` instead of `config.exs` so that the environment variables are read at runtime and not compile time.
